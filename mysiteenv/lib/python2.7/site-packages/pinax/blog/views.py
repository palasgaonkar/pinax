...
class SectionIndexView(BlogIndexView):
    def get_current_section(self):
        section = get_object_or_404(Section,
                                    slug__iexact=self.kwargs.get("section"))
        return section

    def get_queryset(self):
        queryset = super(SectionIndexView, self).get_queryset()
        queryset = queryset.filter(
            section__slug__iexact=self.kwargs.get("section"))
        return queryset
...
class DateBasedPostDetailView(DateDetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if settings.PINAX_BLOG_SLUG_UNIQUE:
            post_redirected.send(sender=self.object, post=self.object,
                                 request=request)
...
class SecretKeyPostDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return super(SecretKeyPostDetailView, self).get(request, *args,
                                                        **kwargs)
...
def blog_feed(request, section=None, feed_type=None):
    ...
    kwargs = {"section": section.slug if section != "all" else "all",
              "feed_type": feed_type}
    feed_url = "http://%s%s" % (
    current_site.domain, reverse(url_name, kwargs=kwargs))
