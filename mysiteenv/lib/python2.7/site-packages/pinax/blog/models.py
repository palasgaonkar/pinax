...
class Post(models.Model):
    ...
    markup = models.CharField(_("Markup"), max_length=25,
                              choices=settings.PINAX_BLOG_MARKUP_CHOICES)
    ...
    published = models.DateTimeField(_("Published"), null=True,
                                     blank=True)  # when last published
    ...
    secret_key = models.CharField(
        ...
        unique=True,
        help_text=_(
            "allows url for sharing unpublished posts to unauthenticated users")
    )
    ...
    def newer_post(self):
        if self.is_published:
            return next(iter(
                Post.objects.published().order_by("published").filter(
                    published__gt=self.published)), None)
    ...
    def sharable_url(self):
        ...
        if not self.is_published or self.is_future_published:
            if self.secret_key:
                return reverse("pinax_blog:blog_post_secret",
        ...
        if self.is_published:
            if settings.PINAX_BLOG_SLUG_UNIQUE:
                ...
                if settings.USE_TZ and settings.TIME_ZONE:
                    published = pytz.timezone(settings.TIME_ZONE).normalize(
                        self.published)
                else:
